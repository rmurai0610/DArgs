...
private:
    ...
    bool get_args(const std::set<std::string> &keys, T& result) {
        if (tokens_.size() == 0) { return false; }
    ...
    template <>
    bool get_args<bool>(const std::set<std::string> &keys, bool& result) {
        if (tokens_.size() == 0) { return false; }
...
public:
    ...
    template <class T>
    T required(const std::string &option_flag, const std::string &short_flag, const std::string &desc) {
        std::string help_key = option_flag + ", " + short_flag;
